...
class AttentionGAN_ck_part2:
    def __init__(self):
        ...
        self.source_roots, self.target_roots = build_pretrain_CK_data(train=True, direction='E2N')
        self.test_source_roots, self.test_target_roots = build_pretrain_CK_data(train=False, direction='E2N')
    ...
    def gen_train_step(self, source, train=True):
        ...
        with tf.GradientTape() as tape:
            gen_img = self.generator_E2N.call(source)
            gen_img2 = self.generator_N2E.call(gen_img)
            v_gen, c_gen = self.discriminator.call(gen_img)
            loss_cycle = reconstruction_loss(source, gen_img2)
            loss_adv = adversarial_loss(v_gen, True)
            loss_cls = classify_loss(label, c_gen)
            loss_g = 10 * loss_cycle + loss_adv + 10 * loss_cls
        if train:
            ...
            return loss_g, 10 * loss_cycle, loss_adv, 10 * loss_cls
    ...
    def dis_train_step(self, source, target, train=True):
        ...
        with tf.GradientTape() as tape:
            gen_img = self.generator_N2E.call(source)
            v_gen, c_gen = self.discriminator.call(gen_img)
    ...
    def train(self, epochs=20, interval=1, batch_size=16, batch_num=36):
        ...
        for epoch in range(epochs):
            ...
            for b in range(batch_num):
                ...
                loss_g_test, loss_cycle, loss_adv_g, loss_cls_g = self.gen_train_step(source_test, train=False)
            ...
            print('Test Loss Gen_cycle      :  {:8.5f}'.format(te_L_G_cycle_avg[-1]))
            ...
            if (epoch % interval == 0 or epoch + 1 == epochs) and (te_L_G_avg[-1] <= np.mean(te_L_G_avg)):
                self.generator_N2E.save_weights('pretrain_weight/ck_generator_N2E_weights_{}'.format(epoch + 1))
                self.generator_E2N.save_weights('pretrain_weight/ck_generator_E2N_weights_{}'.format(epoch + 1))
                self.discriminator.save_weights('pretrain_weight/ck_discriminator_N_weights_{}'.format(epoch + 1))

        return [te_L_G_avg, te_L_G_cycle_avg, te_L_G_adv_avg, te_L_G_cls_avg], \
               [te_L_D_avg, te_L_D_adv_avg, te_L_D_cls_avg], \
               [tr_L_G_avg, tr_L_D_avg]

    def sample_image_pretrain(self, epoch, path='pretrain_picture/ck_pretrain'):
        ...
        gen_img = self.generator_E2N.call(source_sampling)
        gen_img2 = self.generator_N2E.call(source_sampling)
        ...
        for j in range(c):
            ...
            axs[2, j].axis('off')
            cnt += 1
        fig.savefig(path + '_N2E_{}.png'.format(epoch + 1))
        plt.close()
...
if __name__ == '__main__':
    ...
    attention_gan = AttentionGAN_ck_part2()
    attention_gan.generator_N2E.load_weights('pretrain_weight/ck_generator_N2E_weights_24')
    attention_gan.generator_E2N.load_weights('pretrain_weight/ck_generator_E2N_weights_24')
    attention_gan.discriminator.load_weights('3pretrain_weight/ck_discriminator_E2N_weights_10')

    [te_L_G_avg, te_L_G_cycle_avg, te_L_G_adv_avg, te_L_G_cls_avg], [te_L_D_avg, te_L_D_adv_avg, te_L_D_cls_avg], \
    [tr_L_G_avg, tr_L_D_avg] = attention_gan.train(epochs=15, interval=1)

    plt.plot(tr_L_G_avg)
    plt.plot(te_L_G_avg)
    plt.legend(['Train', 'Test'])
    plt.title('CK Generator E2N pretrain loss')
    plt.savefig('pretrain_picture/ck_Generator E2N pretrain loss.jpg')
    plt.close()

    plt.plot(tr_L_D_avg)
    plt.plot(te_L_D_avg)
    plt.legend(['Train', 'Test'])
    plt.title('CK Discriminator E2N pretrain loss')
    plt.savefig('pretrain_picture/ck_Discriminator E2N pretrain loss.jpg')
    plt.close()

    plt.plot(te_L_G_adv_avg)
    plt.plot(te_L_D_adv_avg)
    plt.legend(['Generator', 'Discriminator'])
    plt.title('CK Adversarial E2N pretrain loss')
    plt.savefig('pretrain_picture/ck_Adversarial E2N loss.jpg')
    plt.close()

    plt.plot(te_L_G_cls_avg)
    plt.title('CK Generator E2N pretrain classify loss')
    plt.savefig('pretrain_picture/ck_Generator E2N classify loss.jpg')
    plt.close()

    plt.plot(te_L_D_cls_avg)
    plt.title('CK Discriminator N2E pretrain classify loss')
    plt.savefig('pretrain_picture/ck_Discriminator N2E classify loss.jpg')
    plt.close()

    plt.plot(te_L_G_cycle_avg)
    plt.title('CK Generator N2E pretrain cycle loss')
    plt.savefig('pretrain_picture/ck_Generator N2E Image loss.jpg')
    plt.close()
