...
class AttentionGAN:
    def __init__(self):
        ...
        self.natural_train_roots, self.expression_train_roots = build_pretrain_CK_data(train=True, direction='N2E')
        self.natural_test_roots, self.expression_test_roots = build_pretrain_CK_data(train=False, direction='N2E')

    def gen_train_step(self, natural_source, expression_source , train=True):
        natural_source = tf.cast(natural_source, dtype='float32')
        expression_source = tf.cast(expression_source, dtype='float32')

        natural_label = [0] * natural_source.shape[0]
        natural_label = tf.one_hot(natural_label, depth=2)
        expression_label = [1] * expression_source.shape[0]
        ...
        with tf.GradientTape() as tape:
            ...
            cycle_natural = self.generator_E2N.call(gen_expression)
            cycle_expression = self.generator_N2E.call(gen_natural)
            v_gen_N, c_gen_N = self.discriminator_N.call(gen_natural)
            v_gen_E, c_gen_E = self.discriminator_E.call(gen_expression)
            loss_img = reconstruction_loss(natural_source, gen_natural) + reconstruction_loss(expression_source, gen_expression)
            loss_cycle = reconstruction_loss(natural_source, cycle_natural) + \
                         reconstruction_loss(expression_source, cycle_expression)
            ...
            loss_g = 10 * loss_img + 10 * loss_cycle + loss_adv_N + loss_adv_E + 10 * loss_cls_N + 10 * loss_cls_E
        if train:
        ...
        return 10 * loss_img, 10 * loss_cycle, [loss_adv_N, 10 * loss_cls_N], [loss_adv_E, 10 * loss_cls_E]

    def dis_train_step(self, natural_source, expression_source, train=True):
        natural_source = tf.cast(natural_source, dtype='float32')
        expression_source = tf.cast(expression_source, dtype='float32')
    ...
    def train(self, epochs=20, interval=1, batch_size=16, batch_num=209):
        ...
        for epoch in range(epochs):
            ...
            for b in range(batch_num):
                ...
                [loss_adv_d_N, loss_cls_d_N], [loss_adv_d_E, loss_cls_d_E] \
                    = self.dis_train_step(natural_source, expression_source, train=True)
                ...
                [loss_adv_d_N, loss_cls_d_N], [loss_adv_d_E, loss_cls_d_E] \
                    = self.dis_train_step(natural_source_test, expression_source_test, train=False)
                te_L_D_N.append(loss_adv_d_N + loss_cls_d_N)
                te_L_D_E.append(loss_adv_d_E + loss_cls_d_E)

                for i in range(3):
                    loss_img, loss_cycle, [loss_adv_g_N, loss_cls_g_N], [loss_adv_g_E, loss_cls_g_E] =  \
                        self.gen_train_step(natural_source, expression_source, train=True)
                ...
                loss_img, loss_cycle, [loss_adv_g_N, loss_cls_g_N], [loss_adv_g_E, loss_cls_g_E] = \
                    self.gen_train_step(natural_source_test, expression_source_test, train=False)
                te_L_G_cycle.append(loss_cycle)
                te_L_G_N.append(loss_cycle + loss_adv_g_N + loss_cls_g_N + loss_img)
                te_L_G_E.append(loss_cycle + loss_adv_g_E + loss_cls_g_E + loss_img)
        ...
        return tr_L_G_cycle_avg, te_L_G_cycle_avg, tr_L_G_img_avg, \
                [tr_L_G_N_avg, tr_L_G_adv_N_avg, tr_L_G_cls_N_avg], \
                [tr_L_G_E_avg, tr_L_G_adv_E_avg, tr_L_G_cls_E_avg], \
                [tr_L_D_N_avg, tr_L_D_adv_N_avg, tr_L_D_cls_N_avg], \
                [tr_L_D_E_avg, tr_L_D_adv_E_avg, tr_L_D_cls_E_avg], \
...
if __name__ == '__main__':
    ...
    attention_gan = AttentionGAN()
    attention_gan.generator_N2E.load_weights('pretrain_weight/ck_generator_N2E_weights_10')
    attention_gan.generator_E2N.load_weights('pretrain_weight/ck_generator_E2N_weights_10')
    attention_gan.discriminator_E.load_weights('pretrain_weight/ck_discriminator_E_weights_9')
    attention_gan.discriminator_N.load_weights('pretrain_weight/ck_discriminator_N_weights_10')
    tr_L_G_cycle_avg, te_L_G_cycle_avg, tr_L_G_img_avg, \
        [tr_L_G_N_avg, tr_L_G_adv_N_avg, tr_L_G_cls_N_avg], \
        [tr_L_G_E_avg, tr_L_G_adv_E_avg, tr_L_G_cls_E_avg], \
        [tr_L_D_N_avg, tr_L_D_adv_N_avg, tr_L_D_cls_N_avg], \
        [tr_L_D_E_avg, tr_L_D_adv_E_avg, tr_L_D_cls_E_avg], \
        [te_L_G_N_avg, te_L_G_E_avg], \
        [te_L_D_N_avg, te_L_D_E_avg] = attention_gan.train(epochs=60, interval=1)
